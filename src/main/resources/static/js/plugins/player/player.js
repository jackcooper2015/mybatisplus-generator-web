...
function Player() {
    ...
    function createParts() {
        ...
        lyric.className='lyric-div'
        songInfo.appendChild(lyric)
        myself.lyric = lyric;
    ...
    function showLyric() {
        if (myself.audio.readyState == 4) {
            ...
            function timeFormat(t) {
                ...
                let millSec = (t%6000).toFixed(3).slice(-3)
                return min + ":" + sec + "." +  millSec;
            }

            let cur=timeFormat(currentTime);
            //当前歌词
            let text = "";
            let lyric = myself.playList[myself.nowPlay].lyric;
            let arr = lyric.split("\n");
            for (let i=0;i< arr.length;i++){
                let time = arr[i].match("\\[(.*?)]")[1]
                let next = arr.length>i?arr[i+1].match("\\[(.*?)]")[1]:""
                if(time < cur  && cur < next){
                    text = arr[i].replace("["+time+"]", "");
                    break;
                }
            }
            myself.lyric.innerHTML = text
        }
